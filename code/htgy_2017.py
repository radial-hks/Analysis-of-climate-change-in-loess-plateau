...
def evaporation_cap(matrix,number):
    ...
    med["海拔"] = pool[pool["区站号"]==number]['高程'].values[0]
...
# 测站相关信息读取
pool = pd.read_csv(r"/Users/radial/Public/rocket_launch/name_id.csv")
a = pd.read_csv(r"/Users/radial/Public/rocket/id.csv")

#def main(y):
for y in a.values:
    print(y[0])
    path = r"/Users/radial/Public/ex_data/"+str(y[0])+"_.csv"
    if os.path.exists(path):
        print("文件已存在")
        pass
    else:
        med_data = pd.read_csv(r"/Users/radial/Public/rocket/{}.csv".format(str(y[0])))
        # 1、月总降水量
        meanrl30(med_data)
        # 2、降雨天数平均降水量
        meanrf(med_data)
        ...
        # 数据保存
        res.to_csv(r"/Users/radial/Public/ex_data/"+str(y[0])+"_.csv",index=False)
